 ...
 * ------------------------------------------------------*/
#if defined(STM32_THREAD_SAFE_BAREMETAL_ALLOW_LOCKS) && \
	(STM32_THREAD_SAFE_BAREMETAL_ALLOW_LOCKS != 0)
uint32_t gflag		  = 0;
uint32_t call_counter = 0;
#endif /* defined(STM32_THREAD_SAFE_BAREMETAL_ALLOW_LOCKS) && \
		  (STM32_THREAD_SAFE_BAREMETAL_ALLOW_LOCKS != 0) */
...
/** Implementing mutex from <a
 * href="https://sourceware.org/git/?p=newlib-cygwin.git;a=blob_plain;f=newlib/libc/misc/lock.c">newlib/libc/misc/lock.c</a>
 */
 ...
 * @see https://itanium-cxx-abi.github.io/cxx-abi/abi.html#once-ctor
 *
 * When building for C++, please make sure that <tt>-fno-threadsafe-statics</tt> is not passed to
 * the compiler
 * @{
 */
